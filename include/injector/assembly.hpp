 ...
 */
/*
 *  This version include changes to compile in MinGW, it is not the original
 * version
 */
#pragma once
...
inline void __declspec(naked) make_reg_pack_and_call() {
    asm(R"asm(
                ...
                pusha # Pushes general purposes registers to reg_pack
                pushf # Pushes EFLAGS to reg_pack
                addl $4, 16(%esp) # Add 4 to reg_pack::esp 'cuz of our return pointer, let it be as before this func is called

                # Send reg_pack as parameter
                push %esp
            )asm");
    ...
    asm(R"asm(
                add $4, %esp # cdecl clean-up
                
                # Destructs the reg_pack from the stack
                subl $4, 16(%esp) # Fix reg_pack::esp before popping it (doesn't make a difference though) (+4 because eflags)

                popf # Warning: Do not use any instruction that changes EFLAGS after this (-> sub affects EF!! <-)
                popa
...
inline void __declspec(naked) make_reg_pack_and_call() {
    _asm
    {
        // Construct the reg_pack structure on the stack
                ...
                add [esp+12], 4 // Add 4 to reg_pack::esp 'cuz of our return pointer, let it be as
        ...
        // Destructs the reg_pack from the stack
                sub [esp+12+4], 4     // Fix reg_pack::esp before popping it (doesn't make a difference
          // though) (+4 because eflags)
                popfd // Warning: Do not use any instruction that changes EFLAGS after
              // this (-> sub affects EF!! <-)
                popad
